...
export function DashboardSelection({ onSelect, onTimerClick }: DashboardSelectionProps) {
      ...
      <div className="max-w-4xl w-full">
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-12">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            onClick={() => {
              // debug: ensure handler is invoked when clicking Continue Learning
              // eslint-disable-next-line no-console
              console.log('[DashboardSelection] Continue Learning clicked');
              onSelect('learning');
            }}
          ...
          <motion.button
            ...
            className="bg-card p-8 rounded-2xl shadow-lg border border-border hover:border-blue-500 transition-colors group text-left"
          >
            <div className="w-16 h-16 bg-blue-100 dark:bg-blue-900/30 rounded-2xl flex items-center justify-center mb-6 group-hover:bg-blue-200 dark:group-hover:bg-blue-900/50 transition-colors">
              <Clock className="w-8 h-8 text-blue-600 dark:text-blue-400" />
            </div>
            <h2 className="text-2xl font-bold text-foreground mb-3">Focus Mode</h2>
            <p className="text-muted-foreground">
              Start a focused learning session with timer and goal tracking.
            </p>
          </motion.button>
        </div>
